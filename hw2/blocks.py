...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(mean = 0.0, std = wstd, size=[in_features, out_features])
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # we need to use the chain rule - multiply by the gradient of the current operation
        dx = torch.matmul(dout, self.w.T)
        self.dw += torch.matmul(x.T,dout)
...
class Dropout(Block):
    ...
    def forward(self, x, **kw):
        # Implement the dropout forward pass. Notice that contrary to
        # previous blocks, this block behaves differently a according to the
        # current mode (train/test).
        # ====== YOUR CODE: ======
        if self.training_mode:
            mask_fill_val = (1.0 / (1.0 - self.p)) if self.p != 1 else 0.0
            drop_mask = torch.where(torch.rand(x.shape) <= self.p, 0.0, mask_fill_val)
            out = drop_mask * x
        else:
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = self.grad_cache['drop_mask'] * dout
        # ========================
